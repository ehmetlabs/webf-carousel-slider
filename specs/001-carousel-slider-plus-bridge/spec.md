...
- JS 侧运行环境与现有示例一致，调用代码无需为本次改动做语义调整。
...
1. **Given** 使用者仅知道 `carousel_slider_plus` 的原始 API/参数/事件命名，**When** 按原始名称调用或监听，**Then** 所有调用与事件均可用且名称一致。
...
- **SC-001**: 覆盖清单显示 100% 原始接口/参数/事件均已暴露且命名一致。
- **SC-002**: 使用既有调用代码的示例验证通过率达到 100%（无需改动即可运行）。
